...
Six analyses to validate buy signals and decide whether exit/rebalancing rules add value:
  ...
  4. ETH profit-taking at absolute price levels
     Symmetric to Analysis 2 but for ETH (2 EUR/week DCA).
     ETH ATH ~$4865 (Nov 2021). $5k/$10k scenarios are forward-looking.

  5. ETH MVRV as sell/pause signal
     ETH MVRV peaked at ~2.0 in 2021 (much lower than BTC).
     Do elevated readings predict negative forward returns?

  Final audit summary of all validated/discarded signals.
...
Data sources (all free, no API key needed):
    - BTC/ETH prices  : CoinMetrics community API
    - BTC/ETH MVRV   : CoinMetrics community API
...
def fetch_eth_mvrv() -> pd.DataFrame:
    """ETH MVRV from CoinMetrics community API. Returns DataFrame(date, mvrv)."""
    cache = CACHE_DIR / "eth_mvrv.csv"
    cached = _load_cache(cache)
    if cached is not None:
        print(f"    ETH MVRV: {len(cached)} days from cache")
        return cached

    print("    ETH MVRV: fetching from CoinMetrics...")
    all_rows: list[dict] = []
    url = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
    params: dict = {
        "assets": "eth",
    ...
    if not all_rows:
        raise ValueError("CoinMetrics returned no ETH MVRV data")
    ...
    df = df[["date", "mvrv"]].dropna().sort_values("date").drop_duplicates("date").reset_index(drop=True)
    _save_cache(df, cache)
    print(f"    ETH MVRV: {len(df)} days fetched and cached")
    return df
...
# ---------------------------------------------------------------------------
# ANALYSIS 0 -- Validation of existing buy signals
# ---------------------------------------------------------------------------

def analysis_validate_buy_signals(
    btc_prices: pd.DataFrame,
    eth_prices: pd.DataFrame,
    eth_mvrv: pd.DataFrame,
) -> None:
    sep = "=" * 70

    print(f"\n{sep}")
    print("  ANALYSIS 0: VALIDATION OF EXISTING BUY SIGNALS (2018-2026)")
    print(sep)
    print()
    print("  Re-validates documented signals against historical data.")
    print("  Original validation scripts were removed; this is the canonical re-check.")
    print("  Funding rate signal cannot be re-validated (no free historical API).")
    print()

    # --- BTC prices ---
    btc = btc_prices.set_index("date")["price"]
    btc.index = pd.to_datetime(btc.index)
    btc = btc[~btc.index.duplicated(keep="first")].sort_index()
    btc = btc.loc[START_DATE:END_DATE]

    # --- ETH prices ---
    eth = eth_prices.set_index("date")["price"]
    eth.index = pd.to_datetime(eth.index)
    eth = eth[~eth.index.duplicated(keep="first")].sort_index()

    # --- ETH MVRV ---
    mvrv = eth_mvrv.set_index("date")["mvrv"]
    mvrv.index = pd.to_datetime(mvrv.index)
    mvrv = mvrv[~mvrv.index.duplicated(keep="first")].sort_index()

    eth_merged = pd.DataFrame({"price": eth, "mvrv": mvrv}).dropna().sort_index()
    eth_merged = eth_merged.loc[START_DATE:END_DATE]

    HORIZONS = [7, 30, 90]

    def fwd_ret(prices: pd.Series, mask: pd.Series, horizons: list[int]) -> dict:
        """For each day where mask is True, compute forward returns at each horizon."""
        out: dict[int, dict] = {}
        for h in horizons:
            rets = []
            for date in prices[mask].index:
    ...
    # Baseline: all days
    btc_all_mask = pd.Series(True, index=btc.index)
    btc_baseline = fwd_ret(btc, btc_all_mask, HORIZONS)
    eth_all_mask = pd.Series(True, index=eth_merged.index)
    eth_baseline = fwd_ret(eth_merged["price"], eth_all_mask, HORIZONS)

    # ---- Signal A: BTC crash >= 15% drop in 24h ----
    btc_ret24 = btc.pct_change() * 100
    crash_mask = btc_ret24 <= -15
    crash_fwd  = fwd_ret(btc, crash_mask, HORIZONS)
    n_crash    = int(crash_mask.sum())

    # ---- Signal B: ETH MVRV < 0.8 ----
    mvrv_crit_mask = eth_merged["mvrv"] < 0.8
    mvrv_crit_fwd  = fwd_ret(eth_merged["price"], mvrv_crit_mask, HORIZONS)
    n_mvrv_crit    = int(mvrv_crit_mask.sum())

    # ---- Signal C: ETH MVRV 0.8 - 1.0 ----
    mvrv_low_mask = (eth_merged["mvrv"] >= 0.8) & (eth_merged["mvrv"] < 1.0)
    mvrv_low_fwd  = fwd_ret(eth_merged["price"], mvrv_low_mask, HORIZONS)
    n_mvrv_low    = int(mvrv_low_mask.sum())

    # --- Print results table ---
    print("  Results vs documented values:")
    print()
    print(f"  {'Signal':<30} {'N':>5}  {'7d mean':>8}  {'7d win%':>7}  "
          f"{'30d mean':>8}  {'30d win%':>7}  {'Documented'}")
    print("  " + "-"*30 + "  " + "  ".join(["-"*5, "-"*8, "-"*7, "-"*8, "-"*7, "-"*20]))

    def _row(label: str, n: int, fwd: dict, documented: str) -> None:
        f7  = fwd[7]
        f30 = fwd[30]
        print(f"  {label:<30} {n:>5}  {f7['mean']:>+7.1f}%  {f7['win']:>6.0f}%  "
              f"{f30['mean']:>+7.1f}%  {f30['win']:>6.0f}%  {documented}")

    _row("ALL DAYS (baseline/BTC)",    len(btc),          btc_baseline,  "n/a")
    _row("ALL DAYS (baseline/ETH)",    len(eth_merged),   eth_baseline,  "n/a")
    _row("BTC crash (<=-15% 24h)",     n_crash,           crash_fwd,     "+9.3% 7d / 77% win")
    _row("ETH MVRV critical (<0.8)",   n_mvrv_crit,       mvrv_crit_fwd, "+34% 30d / 89% win")
    _row("ETH MVRV low (0.8-1.0)",     n_mvrv_low,        mvrv_low_fwd,  "positive historical")

    print()
    print("  BTC funding rate (<-0.01%): cannot re-validate")
    print("    No free historical funding rate API available.")
    print("    Documented: +23% at 30d, 88% win rate.")

    # --- Discrepancy check ---
    print()
    print("  Discrepancy check (flag if re-validated vs documented differs >5pp):")
    checks = [
        ("BTC crash 7d mean",    crash_fwd[7]["mean"],      9.3,   5.0),
        ("BTC crash 7d win%",    crash_fwd[7]["win"],       77.0,  10.0),
        ("ETH MVRV<0.8 30d mean", mvrv_crit_fwd[30]["mean"], 34.0,  10.0),
        ("ETH MVRV<0.8 30d win%", mvrv_crit_fwd[30]["win"],  89.0,  10.0),
    ]
    any_discrepancy = False
    for name, measured, documented, tolerance in checks:
        diff = abs(measured - documented)
        if diff > tolerance:
            print(f"    [DISCREPANCY] {name}: measured {measured:+.1f} vs documented {documented:+.1f} "
                  f"(diff {diff:.1f}, tolerance +-{tolerance})")
            any_discrepancy = True
    if not any_discrepancy:
        print("    All signals within expected range. Documented values confirmed.")
    ...
    if crash_fwd[7]["mean"] > btc_baseline[7]["mean"] + 3:
        _verdict_parts.append(
            f"  -> BTC crash signal CONFIRMED: {crash_fwd[7]['mean']:+.1f}% 7d mean "
            f"({crash_fwd[7]['win']:.0f}% win rate) vs baseline {btc_baseline[7]['mean']:+.1f}%"
        )
    else:
        _verdict_parts.append(
            f"  -> BTC crash signal WEAK in this dataset: {crash_fwd[7]['mean']:+.1f}% 7d "
            f"vs baseline {btc_baseline[7]['mean']:+.1f}%"
        )

    if mvrv_crit_fwd[30]["mean"] > eth_baseline[30]["mean"] + 5:
        _verdict_parts.append(
            f"  -> ETH MVRV<0.8 signal CONFIRMED: {mvrv_crit_fwd[30]['mean']:+.1f}% 30d mean "
            f"({mvrv_crit_fwd[30]['win']:.0f}% win rate) vs baseline {eth_baseline[30]['mean']:+.1f}%"
        )
    else:
        _verdict_parts.append(
            f"  -> ETH MVRV<0.8 signal WEAK in this dataset: {mvrv_crit_fwd[30]['mean']:+.1f}% 30d "
            f"vs baseline {eth_baseline[30]['mean']:+.1f}%"
        )

    if mvrv_low_fwd[30]["mean"] > eth_baseline[30]["mean"]:
        _verdict_parts.append(
            f"  -> ETH MVRV 0.8-1.0 signal CONFIRMED: {mvrv_low_fwd[30]['mean']:+.1f}% 30d "
            f"({mvrv_low_fwd[30]['win']:.0f}% win rate) vs baseline {eth_baseline[30]['mean']:+.1f}%"
        )
    else:
        _verdict_parts.append(
            f"  -> ETH MVRV 0.8-1.0 signal WEAK: {mvrv_low_fwd[30]['mean']:+.1f}% 30d "
            f"vs baseline {eth_baseline[30]['mean']:+.1f}%"
        )

    for v in _verdict_parts:
        print(v)
...
# ---------------------------------------------------------------------------
# ANALYSIS 4 -- ETH profit-taking at absolute price levels
# ---------------------------------------------------------------------------

def analysis_eth_profit_taking(eth_prices: pd.DataFrame) -> None:
    sep = "=" * 70

    print(f"\n{sep}")
    print("  ANALYSIS 4: ETH PROFIT-TAKING AT ABSOLUTE PRICE LEVELS")
    print(sep)
    print()
    print(f"  Base strategy: DCA {WEEKLY_EUR['eth']:.0f} EUR/week into ETH since {START_DATE}")
    print(f"  Sell proceeds : kept as cash (not reinvested)")
    print(f"  Fee per sell  : 1 EUR flat (Trade Republic crypto)")
    print()

    prices = eth_prices.set_index("date")["price"]
    prices.index = pd.to_datetime(prices.index)
    prices = prices.loc[START_DATE:END_DATE].dropna().sort_index()
    prices = prices[~prices.index.duplicated(keep="first")]
    ...
    years = (prices.index[-1] - prices.index[0]).days / 365.25
    total_weeks = len(weekly_dates)
    total_invested = total_weeks * WEEKLY_EUR["eth"]

    print(f"  Period        : {prices.index[0].date()} to {prices.index[-1].date()} ({years:.1f} years)")
    print(f"  ETH price     : ${prices.iloc[0]:,.0f} -> ${prices.iloc[-1]:,.0f}  "
          f"(peak: ${prices.max():,.0f})")
    print(f"  Total invested: ~{total_invested:,.0f} EUR over {total_weeks} weeks")
    print()

    # NOTE: ETH has never reached $5k (ATH ~$4865 in Nov 2021).
    # Scenarios at $5k and $10k will show no sells triggered -- this is
    # intentional. They are forward-looking reference points only.
    SCENARIOS: list[tuple[str, list[tuple[float, float]]]] = [
        ("Pure hold (baseline)",               []),
        ("Sell 20% at $3k",                    [(3_000, 0.20)]),
        ("Sell 25% at $3k",                    [(3_000, 0.25)]),
        ("Sell 33% at $3k",                    [(3_000, 0.33)]),
        ("Sell 25% at $3k, 25% at $5k",        [(3_000, 0.25), (5_000, 0.25)]),
        ("Sell 25% at $5k",                    [(5_000, 0.25)]),
        ("Sell 33% at $5k",                    [(5_000, 0.33)]),
        ("Sell 25% at $5k, 25% at $10k",       [(5_000, 0.25), (10_000, 0.25)]),
    ]

    def simulate_eth(levels: list[tuple[float, float]]) -> dict:
        eth_units   = 0.0
        cash        = 0.0
        invested    = 0.0
        total_fees  = 0.0
        ...
        for date, price in prices.items():
            if date in weekly_dates:
                eth_units += WEEKLY_EUR["eth"] / price
                invested  += WEEKLY_EUR["eth"]

            for level, frac in levels:
                if level not in triggered and price >= level:
                    sell_units = eth_units * frac
                    sell_val   = sell_units * price
                    fee        = CRYPTO_SELL_FEE_EUR
                    net        = sell_val - fee
                    eth_units -= sell_units
                    cash      += net
                    total_fees += fee
                    triggered.add(level)
                    sell_log.append({
        ...
        final_eth   = eth_units * prices.iloc[-1]
        final_total = final_eth + cash
        tot_ret     = (final_total - invested) / invested * 100
        ann         = cagr(invested, final_total, years)

        return {
            "invested": invested, "final_eth": final_eth, "cash": cash,
            "final_total": final_total, "total_ret": tot_ret, "cagr": ann,
            "fees": total_fees, "eth_remaining": eth_units, "sell_log": sell_log,
        }

    results = []
    for name, levels in SCENARIOS:
        r = simulate_eth(levels)
        r["name"] = name
        results.append(r)
    ...
    for r in results[1:]:
        if not r["sell_log"]:
            print(f"    [{r['name'][:50]}] : no sells triggered (price never reached)")
        else:
            print(f"    [{r['name'][:50]}]")
            for s in r["sell_log"]:
                print(f"      {s['date'].date()} @ ${s['price']:>8,.0f} : "
    ...
    # --- Cycle breakdown ---
    print()
    print("  ETH price peaks by cycle:")
    cycle_data = [
        ("2018 peak (~$1.4k)",  "2018-01-13", 1_400),
        ("2021 peak (~$4.8k)",  "2021-11-10", 4_865),  # ATH: ~$4865 on 10-Nov-2021
        ("2024 peak (~$4.1k)",  "2024-03-12", 4_100),  # cycle peak March 2024
    ]
    for label, date_str, peak in cycle_data:
        d = pd.Timestamp(date_str)
        idx = prices.index.searchsorted(d)
    ...
    if better:
        best = max(better, key=lambda x: x["total_ret"])
        diff = best["total_ret"] - baseline["total_ret"]
        print(f"  -> Some strategies beat pure hold:")
        print(f"     Best: '{best['name'][:50]}' "
              f"-> {best['total_ret']:+.1f}% vs hold {baseline['total_ret']:+.1f}% ({diff:+.1f}pp)")
        if diff > 20:
            print(f"     SIGNIFICANT advantage (>20pp). Consider adding ETH profit-taking alert.")
        else:
            print(f"     Moderate advantage (<20pp). Marginal benefit vs complexity.")
    else:
        print(f"  -> Pure hold beats ALL profit-taking strategies over this period.")
        print(f"     ETH profit-taking at these levels does NOT add value historically.")
    ...
    print(f"  -> ETH ATH: ~$4865 (Nov 2021). $5k and $10k scenarios are forward-looking.")
...
# ---------------------------------------------------------------------------
# ANALYSIS 5 -- ETH MVRV as sell/pause signal
# ---------------------------------------------------------------------------

def analysis_eth_mvrv_sell(eth_prices: pd.DataFrame, eth_mvrv: pd.DataFrame) -> None:
    sep = "=" * 70

    print(f"\n{sep}")
    print("  ANALYSIS 5: ETH MVRV AS PAUSE/SELL SIGNAL")
    print(sep)
    print()
    print("  Context: BTC MVRV sell signal discarded (only 14 weeks above 3.0 since 2018).")
    print("  ETH MVRV peaked at ~2.0 in the 2021 bull run (~$4865). This analysis")
    print("  checks the high side: do elevated MVRV readings predict negative returns?")
    print("  Already validated BUY signal: MVRV < 0.8 (red) / < 1.0 (yellow).")
    print()
    ...
    mvrv_s = eth_mvrv.set_index("date")["mvrv"]
    mvrv_s.index = pd.to_datetime(mvrv_s.index)
    mvrv_s = mvrv_s[~mvrv_s.index.duplicated(keep="first")]

    merged = pd.DataFrame({"price": prices, "mvrv": mvrv_s}).dropna().sort_index()

    print(f"  MVRV data: {merged.index[0].date()} to {merged.index[-1].date()} ({len(merged)} days)")
    print()
    print("  ETH MVRV distribution:")
    q = merged["mvrv"].quantile([0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99])
    print(f"    Min/Max : {merged['mvrv'].min():.2f} / {merged['mvrv'].max():.2f}")
    print(f"    Mean    : {merged['mvrv'].mean():.2f}  (Median: {merged['mvrv'].median():.2f})")
    print(f"    P90     : {q[0.9]:.2f}   P95: {q[0.95]:.2f}   P99: {q[0.99]:.2f}")
    ...
    print()
    print("  Top 15 ETH MVRV readings (all-time):")
    top = merged.nlargest(15, "mvrv")
    for date, row in top.iterrows():
        print(f"    {date.date()}  MVRV={row['mvrv']:.2f}  ETH=${row['price']:>10,.0f}")
    ...
    for label, (start, end) in cycles.items():
        ...
        peak_row = window.loc[peak_idx]
        print(f"    {label}: peak MVRV={peak_row['mvrv']:.2f} on {peak_idx.date()} "
              f" (ETH=${peak_row['price']:>9,.0f})")

    # --- Forward return analysis ---
    HORIZONS   = [30, 90, 180, 365]
    THRESHOLDS = [1.0, 1.2, 1.5, 2.0, 2.5, 3.0]
    ...
    print()
    print("  Forward return analysis by MVRV threshold (HIGH side):")
    print("  (When MVRV >= X, what happens to ETH over the next N days?)")
    print()
    ...
    # --- Pause DCA simulation ---
    print()
    print("  Pause DCA simulation (pause weekly ETH buy when MVRV >= threshold):")
    print("  Resume: always resume when MVRV drops back below threshold.")
    print("  Cash from paused weeks is kept, not deployed elsewhere.")
    print()
    ...
    def sim_pause(pause_thr: float) -> dict:
        ...
        for date, row in sim_data.iterrows():
            if date not in sim_weeks:
                continue
            if row["mvrv"] >= pause_thr:
                cash_saved += WEEKLY_EUR["eth"]
                invested   += WEEKLY_EUR["eth"]
                paused_wks += 1
            else:
                eth_units += WEEKLY_EUR["eth"] / row["price"]
                invested  += WEEKLY_EUR["eth"]
                active_wks += 1

        last_price   = sim_data["price"].iloc[-1]
        final_eth    = eth_units * last_price
        final_total  = final_eth + cash_saved
        tot_ret      = (final_total - invested) / invested * 100
        ann          = cagr(invested, final_total, sim_years)
        return {
            "final": final_total, "total_ret": tot_ret, "cagr": ann,
    ...
    for date, row in sim_data.iterrows():
        if date in sim_weeks:
            bl_units += WEEKLY_EUR["eth"] / row["price"]
            bl_inv   += WEEKLY_EUR["eth"]
    bl_final  = bl_units * sim_data["price"].iloc[-1]
    bl_return = (bl_final - bl_inv) / bl_inv * 100
    bl_cagr   = cagr(bl_inv, bl_final, sim_years)
    ...
    pause_results = []
    for thr in [1.2, 1.5, 2.0, 2.5, 3.0]:
        r = sim_pause(thr)
        vs = r["total_ret"] - bl_return
        print(f"  {'Pause DCA when MVRV >= '+str(thr):<35}"
              f" | {r['final']:>10,.0f}"
    ...
    print()
    print(f"  Current status  ({current_date.date()}):")
    print(f"    ETH price     : ${current_price:,.0f}")
    print(f"    ETH MVRV      : {current_mvrv:.2f}")

    # --- Verdict ---
    print()
    print("  VERDICT:")

    mvrv15 = next((x for x in thresh_results if x["threshold"] == 1.5), None)
    if mvrv15:
        f30  = mvrv15["fwd"][30]
        f180 = mvrv15["fwd"][180]
        b30  = baseline[30]
        ...
        if f30["win"] < 50 and f30["mean"] < 0:
            print(f"  -> MVRV > 1.5 predicts NEGATIVE 30d returns: "
                  f"{f30['mean']:+.1f}% (win rate {f30['win']:.0f}%)")
            print(f"     vs baseline 30d: {b30['mean']:+.1f}% (win rate {b30['win']:.0f}%)")
        elif delta_30 < -10 and delta_180 < -20:
            print(f"  -> MVRV > 1.5 predicts significantly LOWER returns:")
            print(f"     30d:  {f30['mean']:+.1f}% vs baseline {b30['mean']:+.1f}%  (delta {delta_30:+.1f}%)")
            print(f"     180d: {f180['mean']:+.1f}% vs baseline {b180['mean']:+.1f}%  (delta {delta_180:+.1f}%)")
        else:
            print(f"  -> MVRV > 1.5 does NOT reliably predict underperformance:")
            print(f"     30d:  {f30['mean']:+.1f}% (win {f30['win']:.0f}%) vs baseline {b30['mean']:+.1f}% (win {b30['win']:.0f}%)")
            print(f"     180d: {f180['mean']:+.1f}% (win {f180['win']:.0f}%) vs baseline {b180['mean']:+.1f}% (win {b180['win']:.0f}%)")
    ...
    print()
    mvrv_2021 = merged.loc["2020-01-01":"2022-06-01"]["mvrv"].max()
    mvrv_2024 = merged.loc["2023-01-01":]["mvrv"].max()
    print(f"  Additional context:")
    print(f"    ETH MVRV peak (2021 bull): {mvrv_2021:.2f}")
    print(f"    ETH MVRV peak (2024 cycle): {mvrv_2024:.2f}")
    print(f"    -> Compare with BTC: 3.96 (2021), 2.78 (2024)")
    print(f"    -> ETH MVRV peaks much lower than BTC, consistent with maturing cycles.")
...
def audit_summary() -> None:
    ...
    print(f"\n{sep}")
    print("  SYSTEM AUDIT -- CryptoTrader Advisor (2018-2026)")
    print(sep)
    print()
    print("  BUY SIGNALS (active in Discord bot -- see Analysis 0 for re-validation):")
    print("  ---------------------------------------------------------------------------")
    print(f"  {'Signal':<30} {'Threshold':<16} {'Documented':<24} {'Dedup'}")
    print("  " + "-"*30 + "  " + "  ".join(["-"*16, "-"*24, "-"*6]))
    rows = [
        ("BTC crash",          "<=-15% 24h",    "+9.3% 7d / 77% win rate",    "6h"),
        ("BTC funding neg",    "<-0.01%",        "+23% 30d / 88% win rate",    "24h"),
        ("ETH MVRV critical",  "MVRV < 0.8",     "+34% 30d / 89% win rate",    "24h"),
        ("ETH MVRV low",       "MVRV 0.8-1.0",   "positive historical",        "24h"),
        ("BTC profit level",   ">=$100k price",  "+65pp vs hold (Analysis 2)", "720h"),
    ]
    for name, threshold, documented, dedup in rows:
        print(f"  {name:<30} {threshold:<16} {documented:<24} {dedup}")

    print()
    print("  SELL/REBALANCE ACTIONS (not automated -- manual triggers):")
    print("  ---------------------------------------------------------------------------")
    print("  Annual rebalancing: sell when BTC or ETH exceeds target allocation >10%")
    print("  -> Validated by Analysis 1: +209% vs +164% no rebalance (CAGR 14.7% vs 12.5%)")
    print("  -> Max drawdown improved: -37.8% vs -55.5%")
    print("  -> Cost: ~1 EUR/year in Trade Republic fees")
    print()
    print("  BTC partial profit at $100k (mental rule, not automated):")
    print("  -> Validated by Analysis 2: +65pp advantage in the 2024 cycle")
    print("  -> Now also active as Discord alert (btc_profit_level)")

    print()
    print("  SIGNALS RESEARCHED AND DISCARDED:")
    print("  ---------------------------------------------------------------------------")
    discarded = [
        ("Fear & Greed",       "Extreme fear gives WORSE returns than baseline"),
        ("SMA/RSI/Bollinger",  "None beat buy & hold over 2018-2026"),
        ("Sell after rallies", "Momentum continues +10.8% after +30% rally"),
        ("ETH MVRV > 3.0",    "Never occurred since 2017 (~2.0 peak in 2021 bull)"),
        ("BTC MVRV > 4.0",    "Only 14 weeks above 3.0 since 2018, declining each cycle"),
        ("ETH profit levels",  "See Analysis 4 results above"),
        ("ETH MVRV high",      "See Analysis 5 results above"),
    ]
    for signal, reason in discarded:
        print(f"  {signal:<25} {reason}")

    print()
    print("  DCA PERFORMANCE (2018-2026):")
    print("  ---------------------------------------------------------------------------")
    print("  BTC DCA  : +16.5% annualized CAGR")
    print("  ETH DCA  : +14.2% annualized CAGR")
    print("  S&P 500  : ~10% annualized (benchmark)")

    print()
    print("  CURRENT SPARPLAN (35 EUR/week = 140 EUR/month):")
    print("  ---------------------------------------------------------------------------")
    for k, eur in WEEKLY_EUR.items():
        pct = TARGET_PCT[k] * 100
        print(f"  {k:<10}: {eur:.0f} EUR/week  ({pct:.1f}% target)")
...
def main() -> None:
    ...
    # Analysis 0: validate existing buy signals on historical data
    analysis_validate_buy_signals(btc_prices, eth_prices, eth_mvrv)

    # Analysis 1: portfolio rebalancing (requires yfinance)
    ...
    # Analysis 2: BTC profit-taking at price levels
    analysis_profit_taking(btc_prices)

    # Analysis 3: BTC MVRV as sell/pause signal
    analysis_btc_mvrv(btc_prices, btc_mvrv)

    # Analysis 4: ETH profit-taking at price levels
    analysis_eth_profit_taking(eth_prices)

    # Analysis 5: ETH MVRV as sell/pause signal
    analysis_eth_mvrv_sell(eth_prices, eth_mvrv)
