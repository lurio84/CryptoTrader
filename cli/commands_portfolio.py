"""Portfolio tracker command: add-buy, add-sell, show, history, export, etc.

Each subcommand is a separate private function; cmd_portfolio() is a thin dispatcher.
"""
...
def _parse_date(raw: str | None) -> datetime:
    """Parse YYYY-MM-DD or return current local datetime."""
    return datetime.strptime(raw, "%Y-%m-%d") if raw else datetime.now()


def _load_all_trades() -> list[dict]:
    """Load every UserTrade as plain dicts (safe to use outside the session)."""
...
# ---------------------------------------------------------------------------
# add-buy / add-sell / add-dividend / add-staking / import / export
# ---------------------------------------------------------------------------

def _cmd_add_trade(args: argparse.Namespace, side: str) -> None:
    ...
    print(
        f"Registered: {side.upper()} {args.units:.6f} {asset_upper} "
        f"@ {args.price_eur:.2f} EUR/unit = {total_eur:.2f} EUR total "
        f"(fee: {args.fee_eur:.2f} EUR, source: {args.source})"
    )


def _cmd_add_dividend(args: argparse.Namespace) -> None:
    trade_date = _parse_date(args.date)
    asset_upper = args.asset.upper()
    trade = UserTrade(
        date=trade_date,
    ...
    with get_session() as session:
        session.add(trade)
    print(
        f"Registered: DIVIDEND {asset_upper} {args.amount_eur:.2f} EUR "
        f"on {trade_date.strftime('%Y-%m-%d')}"
    )


def _cmd_add_staking(args: argparse.Namespace) -> None:
    trade_date = _parse_date(args.date)
    trade = UserTrade(
        date=trade_date,
        asset="ETH",
    ...
    print(
        f"Registered: STAKING ETH {args.units:.6f} units @ {args.price_eur:.2f} EUR/unit "
        f"= {total_eur:.2f} EUR"
    )
...
def _cmd_import(args: argparse.Namespace) -> None:
    ...
    if args.dry_run:
        ...
        for r in rows:
            print(
                f"  {r['date'].date()} {r['asset']:<16} {r['side']:<4} "
                f"{r['units']:.6f} @ {r['price_eur']:.2f} EUR  [{r['source']}]"
            )
        return
    ...
    with get_session() as session:
        for r in rows:
            asset_class = r["asset_class"] or detect_asset_class(r["asset"])
            session.add(
                UserTrade(
                    date=r["date"],
                    asset=r["asset"],
...
def _cmd_export(all_trades: list[dict]) -> None:
...
def _print_tax_report_csv(report: dict) -> None:
    import io
    import csv as _csv
    ...
    writer.writerow(
        ["date", "asset", "units", "sale_price_eur", "cost_basis_eur", "proceeds_eur", "gain_eur"]
    )
...
def _cmd_tax_report(args: argparse.Namespace, all_trades: list[dict]) -> None:
    year = args.year or datetime.now().year
    report = calculate_tax_report(all_trades, year)

    if args.csv:
        _print_tax_report_csv(report)
        return
    ...
    print(
        f"  {'Fecha':<12} {'Activo':<16} {'Unidades':>12} "
        f"{'Precio EUR':>11} {'Coste FIFO':>11} {'Ganancia':>11}"
    )
    print("  " + "-" * 70)
    for r in report["rows"]:
        d = r["date"]
        ...
        print(
            f"  {date_str:<12} {r['asset']:<16} {r['units']:>12.6f} "
            f"{r['sale_price_eur']:>10.2f}E {r['cost_basis_eur']:>10.2f}E "
            f"{sign}{r['gain_eur']:>9.2f}E"
        )
    print("  " + "-" * 70)
    ...
    if report["total_gain_eur"] > 0:
        print(
            f"  IRPF ESTIMADO (tramos ES):  {report['total_irpf_eur']:>10,.2f} EUR  "
            f"(~{report['effective_rate_pct']:.1f}% efectivo)"
        )
        for b in report["bracket_breakdown"]:
            print(
                f"    Tramo {b['label']}: {b['rate_pct']:.0f}% sobre "
                f"{b['taxable_eur']:,.2f} EUR -> {b['tax_eur']:,.2f} EUR"
            )
...
def _cmd_history(all_trades: list[dict]) -> None:
    if not all_trades:
        print("No trades registered. Use 'portfolio add-buy' to add your first trade.")
        return
    print(
        f"{'Date':<12} {'Asset':<16} {'Side':<5} {'Units':>12} {'Price EUR':>11} "
        f"{'Fee':>6} {'Source':<12} Notes"
    )
    print("-" * 90)
    for t in sorted(all_trades, key=lambda x: x["date"]):
        ...
        date_str = d.strftime("%Y-%m-%d") if hasattr(d, "strftime") else str(d)
        print(
            f"{date_str:<12} {t['asset']:<16} {t['side']:<5} {t['units']:>12.6f} "
            f"{t['price_eur']:>10.2f}E {t['fee_eur']:>5.2f}E "
            f"{(t['source'] or ''):.<12} {t['notes'] or ''}"
        )


def _cmd_history_chart() -> None:
    import json
    from data.models import UserPortfolioSnapshot

    with get_session() as session:
        snaps = (
            session.query(UserPortfolioSnapshot)
            .order_by(UserPortfolioSnapshot.snapshot_date)
            .all()
        )
    ...
    for s in snap_list:
        print(
            "  {:<12} {:>12,.0f} {:>+10,.0f} {:>+10,.0f} {:>10,.0f}".format(
                s["date"],
                s.get("total", 0),
                s.get("btc_pnl", 0),
                s.get("eth_pnl", 0),
...
# ---------------------------------------------------------------------------
# show (default): crypto + ETF + allocation vs targets
# ---------------------------------------------------------------------------
...
def _print_crypto_block(
    ...
    eth_trades: list[dict],
    btc_price_eur: float | None,
    eth_price_eur: float | None,
    ...
    for asset, trades, price_eur in [
        ...
        print(
            f"    Ganancia no real: {sign}{s['unrealized_gain_eur']:>8,.2f} EUR "
            f"({sign}{s['unrealized_pct']:.1f}%)"
        )
        if s["realized_gain_eur"] != 0:
            print(f"    Ganancia real.:   {s['realized_gain_eur']:>10,.2f} EUR (ventas anteriores)")
        print(
            f"    IRPF si vendieras:{s['irpf_estimate_eur']:>9,.0f} EUR "
            f"(~{s['irpf_rate_pct']:.0f}% efectivo)"
        )
        xirr_val = calculate_xirr(build_xirr_cash_flows(trades, price_eur))
        if xirr_val is not None:
            print(
                f"    IRR anualizada:   {xirr_val * 100:>+9.1f}%  "
                "(TIR considerando fechas y cash flows)"
            )
        if s["next_dca_level_eur"]:
            pct_to_level = (s["next_dca_level_eur"] / price_eur - 1) * 100
            print(
                f"    Proximo DCA-out:  {asset} a {s['next_dca_level_eur']:,.0f} EUR "
                f"({pct_to_level:+.1f}%) -> vende {s['next_dca_units']:.6f} {asset} "
                f"({s['next_dca_eur']:,.0f} EUR)"
            )
    ...
    return crypto_total_invested, crypto_total_value, crypto_unrealized, crypto_irpf


def _print_etf_block(
    etf_trades_all: list[dict], etf_prices: dict
    ...
    if not etf_assets:
        ...
        return etf_value_map, etf_invested_total, etf_value_total
    ...
    for asset in etf_assets:
        ...
        if current_price is not None and total_units > 0:
            ...
            print(f"\n  {asset}: {total_units:.4f} u  ({len(trades_for)} operaciones)")
            print(
                f"    Precio no disponible (yfinance sin conexion). "
                f"Invertido: {total_invested:,.2f} EUR"
            )
            etf_invested_total += total_invested
...
def _print_allocation_block(
    ...
    all_values: dict[str, float] = {
        "BTC": 0.0, "ETH": 0.0,
        "SP500": 0.0, "SEMICONDUCTORS": 0.0,
        "REALTY_INCOME": 0.0, "URANIUM": 0.0,
    }
    ...
    print(
        f"\n  {'Activo':<16} {'Valor EUR':>10}  {'Actual%':>7}  "
    ...
    for asset, target_pct in SPARPLAN_TARGETS.items():
        ...
        if abs(drift) > DRIFT_THRESHOLD:
            ...
            estado = "[OK]"
        print(
            f"  {asset:<16} {val:>10,.0f}E  {actual_pct:>6.1f}%  "
...
def _cmd_show(all_trades: list[dict]) -> None:
    prices = fetch_portfolio_prices_eur(include_etfs=True)
    btc_price_eur = prices["btc_eur"]
    eth_price_eur = prices["eth_eur"]
    ...
    (
        crypto_total_invested,
        crypto_total_value,
        crypto_unrealized,
        _crypto_irpf,
    ) = _print_crypto_block(btc_trades, eth_trades, btc_price_eur, eth_price_eur)

    etf_value_map, etf_invested_total, etf_value_total = _print_etf_block(
        etf_trades_all, etf_prices
    )

    total_portfolio = _print_allocation_block(
        btc_trades, eth_trades, btc_price_eur, eth_price_eur, etf_value_map
    )
    ...
    print(
        f"\n  Crypto invertido: {crypto_total_invested:>10,.2f} EUR  |  "
        f"Valor: {crypto_total_value:>10,.2f} EUR  |  "
        f"PnL: {crypto_sign}{crypto_unrealized:,.2f} EUR"
    )
    if etf_value_total > 0:
        etf_pnl = etf_value_total - etf_invested_total
        etf_sign = "+" if etf_pnl >= 0 else ""
        print(
            f"  ETF invertido:    {etf_invested_total:>10,.2f} EUR  |  "
            f"Valor: {etf_value_total:>10,.2f} EUR  |  "
            f"PnL: {etf_sign}{etf_pnl:,.2f} EUR"
        )
    ...
    if income_trades:
        ...
        income_year = [
            t for t in income_trades if hasattr(t["date"], "year") and t["date"].year == year_now
        ]
        total_income = sum(t["price_eur"] for t in income_year)
        if total_income > 0:
            print(
                f"\n  Dividendos+Staking {year_now}: {total_income:,.2f} EUR "
                f"(retencion estimada 19%: {total_income * 0.19:,.2f} EUR)"
            )
...
def cmd_portfolio(args: argparse.Namespace) -> None:
    ...
    if sub in ("add-buy", "add-sell"):
        _cmd_add_trade(args, side="buy" if sub == "add-buy" else "sell")
        return
    if sub == "add-dividend":
        _cmd_add_dividend(args)
        return
    if sub == "add-staking":
        _cmd_add_staking(args)
        return
    if sub == "import":
        _cmd_import(args)
        return
    ...
    # Subcommands that need the full trade list
    all_trades = _load_all_trades()

    if sub == "export":
        _cmd_export(all_trades)
        return
    if sub == "tax-report":
        _cmd_tax_report(args, all_trades)
        return
    if sub == "history":
        _cmd_history(all_trades)
        return

    # Default: show
    _cmd_show(all_trades)
...
def cmd_tax_headroom(args: argparse.Namespace) -> None:
    ...
    if btc_trades and btc_price_eur:
        s = calculate_portfolio_status(
            "BTC", btc_trades, btc_price_eur, BTC_DCA_OUT_BASE, BTC_DCA_OUT_STEP
        )
        btc_unrealized = max(s["unrealized_gain_eur"], 0.0)
    if eth_trades and eth_price_eur:
        s = calculate_portfolio_status(
            "ETH", eth_trades, eth_price_eur, ETH_DCA_OUT_BASE, ETH_DCA_OUT_STEP
        )
    ...
    if realized <= 0:
        ...
        print(
            "  IRPF sobre realizadas:    {:>10,.0f} EUR ({:.0f}%)".format(
        ...
        if headroom_info["headroom_eur"] is not None:
            print(
                "  Margen hasta sig. tramo:  {:>10,.0f} EUR".format(headroom_info["headroom_eur"])
            )
        else:
    ...
    if total_unrealized > 0:
        ...
        print(
            "  Si vendieras todo BTC+ETH hoy:      {:>8,.0f} EUR adicionales".format(
                total_unrealized
            )
        )
        ...
        print(
            "  Sin plusvalias no realizadas detectadas (precio no disponible o posicion en perdida)."
        )
    print()
...
def _maybe_notify_tax_headroom(
    ...
    if headroom_eur is None:
        print("  [--notify] Tramo maximo alcanzado -- sin margen que notificar.")
        return
    if headroom_eur >= threshold:
        print(
            "  [--notify] Margen {:,.0f} EUR >= threshold {:,.0f} EUR -- sin alerta.".format(
    ...
    init_db()
    with get_session() as session:
        if _already_alerted(session, "tax_headroom_low", COOLDOWN_TAX_HEADROOM):
        ...
        if sent:
            print(
                "  [Discord] Alerta enviada: margen {:,.0f} EUR < {:,.0f} EUR threshold.".format(
                    headroom_eur, threshold
                )
            )
        else:
